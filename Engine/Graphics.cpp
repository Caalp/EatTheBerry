...
void Graphics::DrawSprite(int x, int y, const Rect& rect, const Surface & s, Color chroma)
{
	const int width = s.GetWidth();
	const int height = s.GetHeight();
	for (int sy =rect.top ; sy < rect.bottom; sy++)
	{
		for (int sx = rect.right; sx < rect.left; sx++)

		{
			if (s.GetPixel(sx, sy) != chroma)
			{
				PutPixel(x + sx-rect.right, y + sy-rect.top, s.GetPixel(sx, sy));
			}
		}
	}
}

void Graphics::DrawLine(Vec2i lhs, Vec2i rhs, Color color)
{
	if (lhs.x > rhs.x)
	{
		std::swap(lhs.x, lhs.y);
		std::swap(rhs.x, rhs.y);
	}
	int dy = rhs.y - lhs.y;
	int dx = rhs.x - lhs.x;
	int p = 2 * dy - dx;
	//int p_next = p + 2 * dy - 2 * dx*("yk+1-yk");
	int m = (rhs.y - lhs.y) / (rhs.x - lhs.x);
...
void Graphics::DrawRect(const Vec2i& v1, const Vec2i& v2, Color c)
{
